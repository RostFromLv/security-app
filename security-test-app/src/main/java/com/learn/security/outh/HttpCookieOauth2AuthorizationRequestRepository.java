...
public class HttpCookieOauth2AuthorizationRequestRepository implements
  ...
  public void saveAuthorizationRequest(OAuth2AuthorizationRequest authorizationRequest,
    ...
    CookieUtils.addCookie(response, oauth2AuthorizationRequestCookieName,
        CookieUtils.serialize(authorizationRequest), cookieExpiredSeconds);
    String redirectUriAfterLogin = request.getParameter(redirectUriParamCookieName);
    if (StringUtils.isNotBlank(redirectUriAfterLogin)) {
      CookieUtils.addCookie(response, oauth2AuthorizationRequestCookieName, redirectUriAfterLogin,
